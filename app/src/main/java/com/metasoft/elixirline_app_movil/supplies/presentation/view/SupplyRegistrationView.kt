...
@Composable
fun SupplyRegistrationView(
    viewModel: SupplyRegistrationViewModel = PresentationModule.getSupplyRegistrationViewModel(),
    onSuccess: () -> Unit,
    onCancel: () -> Unit
) {
    ...
    LaunchedEffect(uiState) {
        when (uiState) {
            is SupplyRegistrationViewModel.UiState.Success -> {
                snackbarHostState.showSnackbar("Insumo registrado correctamente")
                onSuccess()
            }
            is SupplyRegistrationViewModel.UiState.Error -> {
                snackbarHostState.showSnackbar((uiState as SupplyRegistrationViewModel.UiState.Error).message)
            }
            else -> {}
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            Column(
                ...
                Button(
                    onClick = { viewModel.registerSupply() },
                    modifier = Modifier.fillMaxWidth(),
            ...
            if (uiState is SupplyRegistrationViewModel.UiState.Loading) {
                Box(
